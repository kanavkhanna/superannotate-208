  ...
  return (
    <div className="space-y-6">
      <Button
        variant="ghost"
        onClick={onBack}
        className="group flex items-center gap-2 mb-4 pl-0 text-[#5E3A21] hover:bg-[#FDF6EC] hover:text-[#8B5A3C]"
      >
        <ArrowLeft className="h-4 w-4 transition-transform group-hover:-translate-x-1" />
        Back to list
      </Button>

      <div className="relative h-64 md:h-[400px] rounded-xl overflow-hidden shadow-md">
        <Image
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6">
          ...
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex items-center text-white">
              <Star className="h-5 w-5 fill-[#F9A826] text-[#F9A826] mr-1" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2">
          <div className="flex flex-wrap gap-2 mb-4">
            {shop.amenities.wifi && (
              <Badge
                variant="secondary"
                className="flex items-center gap-1 bg-[#FDF6EC] text-[#8B5A3C] hover:bg-[#E6C9A8]"
              >
          ...
          <p className="text-[#8B5A3C] mb-6">{shop.description}</p>

          <Tabs defaultValue="info" className="w-full">
            <TabsList className="w-full grid grid-cols-3 bg-[#FDF6EC]">
              <TabsTrigger value="info" className="data-[state=active]:bg-[#5E3A21] data-[state=active]:text-white">
                Info
              </TabsTrigger>
              <TabsTrigger value="map" className="data-[state=active]:bg-[#5E3A21] data-[state=active]:text-white">
                Map
              </TabsTrigger>
              <TabsTrigger value="reviews" className="data-[state=active]:bg-[#5E3A21] data-[state=active]:text-white">
                Reviews
              </TabsTrigger>
            </TabsList>
            <TabsContent value="info" className="space-y-4 mt-4">
              <div className="space-y-4">
                <h3 className="text-xl font-semibold text-[#5E3A21]">About {shop.name}</h3>
                <p className="text-[#8B5A3C]">{shop.longDescription}</p>

                <Separator className="bg-[#E6C9A8]" />

                <h3 className="text-xl font-semibold text-[#5E3A21]">Specialties</h3>
                <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {shop.specialties.map((specialty, index) => (
                    <li key={index} className="flex items-center gap-2 text-[#8B5A3C]">
                      <div className="h-1.5 w-1.5 rounded-full bg-[#5E3A21]"></div>
                      {specialty}
                    </li>
                  ))}
                </ul>
            ...
            <TabsContent value="map" className="mt-4">
              ...
              <div className="mt-4 flex justify-center">
                <Button
                  variant="outline"
                  className="gap-2 transition-all border-[#E6C9A8] text-[#5E3A21] hover:bg-[#FDF6EC] hover:text-[#8B5A3C]"
                  onClick={() => {
        ...
        <div className="space-y-4">
          <div className="rounded-lg bg-white p-4 shadow-sm">
            <h3 className="font-medium mb-3 text-[#5E3A21]">Hours</h3>
          ...
          <div className="rounded-lg bg-white p-4 shadow-sm">
            <h3 className="font-medium mb-3 text-[#5E3A21]">Location</h3>
            <div className="space-y-2 text-[#8B5A3C]">
          ...
          <div className="rounded-lg bg-white p-4 shadow-sm">
            <h3 className="font-medium mb-3 text-[#5E3A21]">Contact</h3>
            <Button
              variant="default"
              className="w-full transition-all bg-[#5E3A21] hover:bg-[#8B5A3C] active:scale-[0.98]"
              onClick={() => {
                // Show a toast with just the phone number highlighted
                toast.success("Coffee Shop Contact", {
                  description: (
                    <div className="flex items-center gap-2">
                      <Phone className="h-4 w-4 text-[#F9A826]" />
                      <span>
                        Contact <span className="font-semibold text-[#F9A826]">555-123-4567</span>
                      </span>
                    </div>
                  ),
                  action: {
